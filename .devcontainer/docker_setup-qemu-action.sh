...
docker image inspect tonistiigi/binfmt:latest
docker run --rm --privileged tonistiigi/binfmt:latest --uninstall "${PLATFORM_ARCH}" || true
docker run --rm --privileged tonistiigi/binfmt:latest --install "${PLATFORM_ARCH}"
exec docker run --rm --privileged tonistiigi/binfmt:latest
